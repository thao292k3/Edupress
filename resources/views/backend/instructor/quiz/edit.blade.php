                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="pass_score" class="form-control @error('pass_score') is-invalid @enderror" 
                               min="0" max="100" value="{{ old('pass_score', $quiz->pass_score) }}" required>
                        @error('pass_score')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="duration_minutes" class="form-control @error('duration_minutes') is-invalid @enderror" 
                               min="1" value="{{ old('duration_minutes', $quiz->duration_minutes) }}">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                            <option value="0" {{ old('status', $quiz->status) == 0 ? 'selected' : '' }}>Draft</option>
                            <option value="1" {{ old('status', $quiz->status) == 1 ? 'selected' : '' }}>Active</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
